...
// Run this alongside `npm run dev` for local email testing
...
app.post('/api/send-doctor-email', async (req, res) => {
  try {
    const { doctorEmail, quizScore, quizData, summary, patientId } = req.body;
    ...
    if (!process.env.GMAIL_CLIENT_ID || !process.env.GMAIL_CLIENT_SECRET || !process.env.GMAIL_REFRESH_TOKEN) {
      ...
      return res.status(500).json({ 
        message: 'Email service is not configured. Please set Gmail OAuth2 credentials in .env file.' 
      });
    }
...
function createEmailTemplate(patientId, quizScore, quizData, summary) {
...
app.listen(PORT, () => {
  ...
  console.log(`📧 Email endpoint: http://localhost:${PORT}/api/send-doctor-email`);
  console.log(`\n💡 Make sure Gmail OAuth2 credentials are set in your .env file`);
